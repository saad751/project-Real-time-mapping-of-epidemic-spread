...
@app.route('/fetch_heatmap_data', methods=['GET'])
def fetch_heatmap_data():

        # Connect to the SQLite database
        conn = sqlite3.connect('epidemic.db')  # Use your actual database file
        cursor = conn.cursor()

        # Query to fetch latitude, longitude, and cases from the 'case_data' table
        query = "SELECT latitude, longitude, cases FROM  Cases"
        cursor.execute(query)
        data = cursor.fetchall()

        # Format the data for heatmap visualization
        heatmap_data = [{"lat": row[0], "lng": row[1], "weight": row[2]} for row in data]
...
def fetch_only_cases():
    ...
    final_cases_array = []  # Move this outside the loop
    final_death_array = []  # Move this outside the loop
    ...
    # Now build the arrays after processing all data
